---
title: "GREEEN tallgrass prairie restoration: Seeded richness ~ Seeding time * Herbicide pre-treatment (NW Station)"
author: "MB"
...
load(file = here("outputs", "models", "model_richness_timing_herbicide_nws_2.Rdata"))
m_1 <- m1
m_2 <- m2
```
...
plotResiduals(
  simulation_output_2$scaledResiduals,
  factor(sites %>% filter(!(is.na(water_cap))) %>%  pull(seeded_pool))
  )
plotResiduals(
  simulation_output_1$scaledResiduals,
...
```{r effect-sizes-1, message = FALSE, warning = FALSE}
(emm <- emmeans(
  m_1,
