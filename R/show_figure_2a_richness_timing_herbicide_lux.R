...
### * Model ####
load(file = here("outputs", "models", "model_richness_timing_herbicide_lux_1.Rdata"))
m <- m1
formula(m)
...
graph_a <- ggplot() +
    ...
    #   ) +
    facet_grid(~group) +
    scale_y_continuous(limits = c(0, 17), breaks = seq(0, 21, 5)) +
    scale_color_manual(
    ...
    labs(
      x = "", y = expression(Seeded ~ forbs ~ "[" * '#' * "]"),
      title = "Lux Arbor"
    ) +
    theme_mb() +
    theme(
      axis.text.x = element_text(
        angle = 90, vjust = 0.5, hjust = 0, lineheight = 0.75
        )
      ); graph_a

### Save ###
ggsave(
  here("outputs", "figures", "figure_2a_300dpi_16x5cm.tiff"),
